...
void writeGifs(MultiBlockLattice3D<T,DESCRIPTOR>& lattice, plint iter,T rho_in, T rho_out)
{
    ...
    const T transglote = 1.1*(rho_in - rho_out);
    const T UpBound = rho_in+transglote;
    const T LowBound = rho_out-transglote ;
...
////////////////////////////////////VARIAVEIS//////////////////////////////////////
    ...
    const plint nx = 432+BoudaryLines+ 0*3*ExtensaoDominio;
    ...
    const T rhoTg_LB = (Ptg_Pa / rho0_ar)*((dt*dt)/(dx*dx)) / (c_LB*c_LB)*20 + rho0_LB  ; /// OLHA ESSE 10
...
/////////////////////////////////CRIAÇAO DO LATTICE////////////////////////////////
    ...
    //MRTparam<T, Descriptor> mrt_param = new MRTparam<T,DESCRIPTOR>(omega);                 // MRT
