...
/* *************** Class AnechoicDynamics *********************************************** */

template<typename T, template<typename U> class Descriptor>
int AnechoicDynamics<T,Descriptor>::id =
    meta::registerOneParamDynamics<T,Descriptor,AnechoicDynamics<T,Descriptor> >("Anechoic");

/** \param omega_ relaxation parameter, related to the dynamic viscosity
 */
template<typename T, template<typename U> class Descriptor>
AnechoicDynamics<T,Descriptor>::AnechoicDynamics(T omega_)
    : IsoThermalBulkDynamics<T,Descriptor>(omega_)
{this->delta = (T) 30;}

template<typename T, template<typename U> class Descriptor>
AnechoicDynamics<T,Descriptor>* AnechoicDynamics<T,Descriptor>::clone() const {
    return new AnechoicDynamics<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
int AnechoicDynamics<T,Descriptor>::getId() const {
    return id;
}

template<typename T, template<typename U> class Descriptor>
void AnechoicDynamics<T,Descriptor>::collide(Cell<T,Descriptor>& cell, BlockStatistics& statistics){
    T rhoBar;
    Array<T,Descriptor<T>::d> j;
    momentTemplates<T,Descriptor>::get_rhoBar_j(cell, rhoBar, j);
...
template<typename T, template<typename U> class Descriptor>
T AnechoicDynamics<T,Descriptor>::computeEquilibrium(plint iPop, T rhoBar, Array<T,Descriptor<T>::d> const& j,
                                                T jSqr, T thetaBar) const
{
    T invRho = Descriptor<T>::invRho(rhoBar);
    return dynamicsTemplates<T,Descriptor>::bgk_ma2_equilibrium(iPop, rhoBar, invRho, j, jSqr);
...
template<typename T, template<typename U> class Descriptor>
void AnechoicDynamics<T,Descriptor>::setDelta(T delta){
    this->delta = delta;
}
...
template<typename T, template<typename U> class Descriptor>
void AnechoicDynamics<T,Descriptor>::decomposeOrder0 (
        Cell<T,Descriptor> const& cell, std::vector<T>& rawData ) const
{
    T rhoBar;
    Array<T,Descriptor<T>::d> j;
...
template<typename T, template<typename U> class Descriptor>
void AnechoicDynamics<T,Descriptor>::recomposeOrder0 (
        Cell<T,Descriptor>& cell, std::vector<T> const& rawData ) const
{
    T rhoBar = rawData[0];
    Array<T,Descriptor<T>::d> j;
